...
$__System.register('c', ['7', '9', 'a', 'b'], function (_export) {
	...
	// ----------------- MENU SHOW AND HIDE -----------------
	function show_menu() {
		// Get header height to dynamically set margin top of body and drawer
		headerHeight = $('.header').outerHeight();
		if (!$('body').hasClass('menuB1') && !$('body').hasClass('menuB2')) {
			$menuDrawer.css({ 'padding-top': headerHeight });
		}
		$('body').css({ 'margin-top': headerHeight });
		// Set position and class of the close X
		if (!$('body').hasClass('menuB1') && !$('body').hasClass('menuB2')) {
			$menuDrawerX.css({ 'top': headerHeight + 26 });
		}
		$menuDrawerX.addClass('active');
		// Set classes
		...
		// Reset scroll top of menu
		$menuDrawer.scrollTop(0);
		// Reset all levels
		$rowGroup.scrollTop(0);
		// Set state
		menuState = 1;
	...
	function hide_menu() {
		// Reset margin top of body
		$('body').css({ 'margin-top': 0 });
		// Reset level and slider
		level = 0;
		$slider.css({ 'left': 0 });
		// Remove classes
		...
		$menuDrawerX.removeClass('active');
		$overlay.removeClass('active');
		$('body').removeClass('noscroll');
		$backBar.removeClass('active');
		// Set state
		menuState = 0;
	}
	// ----------------- MENU SHIFT FORWARD -----------------
	function shift_menu_forward() {
		// Increase level
		level++;
		// Move slider forward
		$slider.css({ 'left': '-' + 100 * level + '%' });
		// Set child group to display
		var $newGroup = $('.group[data-group="' + child + '"]');
		// Remove any previous group with this level class
		$('.level' + level).removeClass('active');
		$('.level' + level).removeClass('level' + level);
		// Add level class and make active
		$newGroup.addClass('level' + level);
		$newGroup.addClass('active');
		// Set the new group into the next position to slide to
		$newGroup.css({ 'top': 0, 'left': 100 / 20 * level + '%' });
		// Set scroll top to 0 in case this user goes backwards then forwards to same child menu
		$newGroup.scrollTop(0);
		// Check if back bar should be visible or not
		toggle_back_bar();
	}
	// ----------------- MENU SHIFT BACK -----------------
	function shift_menu_back() {
		// Decrease level
		level--;
		// Move slider back
		$slider.css({ 'left': '-' + 100 * level + '%' });
	...
	// ----------------- MENU SHIFT BACK -----------------
	function toggle_back_bar() {
		if (level > 0) {
			$backBar.addClass('active');
		} else {
			$backBar.removeClass('active');
		}
	}
	...
	return {
		...
		execute: function () {
			...
			$menuDrawerX = $('.menu-drawer-x');
			$slider = $menuDrawer.find('.menu-slider');
			$menuLink = $menuDrawer.find('.row');
			$backBar = $menuDrawer.find('.back-bar');
			$menuBack = $menuDrawer.find('.icon-back');
			$header = $('.header');
			$row = $menuDrawer.find('.row');
